 ...
 */
struct request {
  uint32_t item_id___stack_item;
  uint32_t item_id___target_item;
...
void packet_d2gs_d2gs_stackitem(struct connection *conn, struct packet *request) {
  ...
  packet.item_id___stack_item = packet_get_int(request, ENDIAN_LITTLE);
  packet.item_id___target_item = packet_get_int(request, ENDIAN_LITTLE);
  ...
  EINA_LOG_CRIT("packet_d2gs_d2gs_stackitem not yet implemented."); packet_free(response); return;
